...
export async function POST(req: NextRequest) {
  ...
  if (!projectSlug || !datasetSlug) {
    return NextResponse.json({ error: 'projectSlug and datasetSlug are required' }, { status: 400 })
  }

  // Only allow saves when the project has publicDemo set to 'editable'
  ...
  if (projectData.publicDemo !== 'editable') {
