...
export async function POST(req: NextRequest) {
  try {
    ...
    // Uses only the email filter + client-side date check to avoid needing a composite index
    try {
      ...
      const recentCount = recentSnap.docs.filter(
        d => d.data().createdAt?.toMillis?.() >= windowStart
      ).length
      if (recentCount >= 5) {
        return NextResponse.json({ ok: true })
      }
    } catch {
      // If index is missing or query fails, skip rate limit rather than block login
    }
